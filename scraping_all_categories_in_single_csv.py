...
for container in containers:
    ...
    hyperlink_cat = container.a["href"]
    hp = hyperlink_cat
    page = 1
    a,b = "",""
    while True:
        ...
        if a!=b:
            ...
            for item in range(len(sub_containers)):
                ...
                Address = Address.replace('\ufeff','')
                Address = Address.replace('\u200b','')
                Pincode = pin_containers[item].text.strip().replace(',','')
                ...
                if(web!=[]):
                    Website = web[0].text.strip().replace(',','')
                
                Email =""
                mail = page_soup_item.findAll('div',{"class":"custom-field custom-field-1"})
                if(mail!=[]):
                    Email = mail[0].a.text.strip().replace(',','')
